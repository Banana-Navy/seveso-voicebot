...
assert.equal(settings.voice.model_id, 'eleven_v3_conversational');
assert.ok(settings.voice.stability >= 0.3 && settings.voice.stability <= 0.5);
assert.deepEqual(settings.active_tool_ids, []);
assert.match(agentPrompt, /Ne dis jamais qu’il existe un problème technique/);
